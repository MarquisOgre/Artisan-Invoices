...
const OutwardRegister = () => {
  ...
  const [viewingEntry, setViewingEntry] = useState<OutwardEntry | null>(null);
  
  // Form state
  const [toParty, setToParty] = useState("");
  const [entryDate, setEntryDate] = useState(currentDate.toISOString().split('T')[0]);
  const [notes, setNotes] = useState("");
  const [items, setItems] = useState<OutwardItem[]>([{ product_name: "", size: "", quantity: 0 }]);
  ...
  const loadEntries = async () => {
    try {
      ...
      const typedData = (data || []).map(entry => ({
        ...entry,
        items: (entry.items as unknown as OutwardItem[]) || []
      }));
  ...
  const generateOutwardNumber = async () => {
    const { data } = await supabase
      .from('outward_register')
      .select('outward_number')
      .order('created_at', { ascending: false })
      .limit(1);
    
    if (data && data.length > 0) {
      const lastNumber = parseInt(data[0].outward_number.replace('OUT-', '')) || 0;
      return `OUT-${String(lastNumber + 1).padStart(4, '0')}`;
    }
    return 'OUT-0001';
  };
  ...
  const handleViewEntry = (entry: OutwardEntry) => {
  ...
  const handleSaveEntry = async () => {
    ...
    try {
      ...
      if (editingEntry) {
        ...
        if (error) throw error;
        toast({ title: "Entry updated", description: "Outward entry has been updated successfully." });
      } else {
        ...
        if (error) throw error;
        toast({ title: "Entry added", description: `Outward ${outwardNumber} has been created.` });
      }
      ...
      toast({ title: "Error", description: "Failed to save outward entry.", variant: "destructive" });
    }
  };
  ...
  const handleDeleteEntry = async (id: string) => {
    try {
      ...
      if (error) throw error;
      toast({ title: "Entry deleted", description: "Outward entry has been removed." });
      loadEntries();
    } catch (error) {
      console.error('Error deleting outward entry:', error);
      toast({ title: "Error", description: "Failed to delete outward entry.", variant: "destructive" });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="w-full" id="outward-register-print">
        <CardHeader>
          <div className="print-title">Outward Register - {MONTHS.find(m => m.value === selectedMonth)?.label} {selectedYear}</div>
          <div className="flex flex-wrap items-center justify-between gap-4 no-print">
            ...
            <CardTitle className="text-xl font-bold text-center flex-1">Outward Register</CardTitle>
            <div className="flex gap-2">
              <Button onClick={handleAddEntry} size="sm"><Plus className="h-4 w-4 mr-2" />New Outward</Button>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b-2 border-border">
                  <th className="text-left p-3 font-semibold">Outward No.</th>
                  <th className="text-left p-3 font-semibold">Date</th>
                  <th className="text-left p-3 font-semibold">To Party</th>
              ...
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id} className="border-b border-border hover:bg-muted/50">
                    <td className="p-3 font-medium">{entry.outward_number}</td>
                    <td className="p-3">{new Date(entry.entry_date).toLocaleDateString()}</td>
                    <td className="p-3">{entry.to_party || "-"}</td>
                ...
                {entries.length === 0 && (
                  <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">No outward entries for this month</td></tr>
                )}
              </tbody>
              <tfoot>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingEntry ? `Edit ${editingEntry.outward_number}` : "New Outward Entry"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input value={toParty} onChange={(e) => setToParty(e.target.value)} placeholder="Enter party name" />
              </div>
            </div>
      ...
      <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{viewingEntry?.outward_number} Details</DialogTitle>
          </DialogHeader>
          {viewingEntry && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="text-muted-foreground">Date:</span> {new Date(viewingEntry.entry_date).toLocaleDateString()}</div>
                <div><span className="text-muted-foreground">To Party:</span> {viewingEntry.to_party || "-"}</div>
              </div>
              <div>
